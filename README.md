...
5. 按 P1、P2 或 P3。
6. 對準畫面後按「拍照」。
7. 照片處理完成後，按「儲存 P1 圖片」、「儲存 P2 圖片」或「儲存 P3 圖片」。
8. 快速模式預設開啟，按儲存後會自動清除預覽。
...
- 手機瀏覽器沒有標準的「預先允許多張自動下載」網頁權限；連續自動下載可能被 Android Chrome 或 iPhone Safari 擋下，所以每張圖片仍保留一次明確的儲存動作。
